...
def get_batch_status(batch_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        cursor.execute(
            "SELECT status, COUNT(*) as count FROM cv_jobs WHERE batch_id = ? GROUP BY status",
            (batch_id,),
        )
...
def get_batch_results(batch_id: str) -> List[Dict[str, Any]]:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, request_id, status, ai_response, error_log FROM cv_jobs WHERE batch_id = ?",
            (batch_id,),
        )
        rows = cursor.fetchall()
...
@retry(
    ...
    retry=retry_if_exception_type(
        Exception
    ),  # You can refine this to specific Gemini API errors if needed
    ...
    while True:
        try:
            ...
            if now - last_cleanup > 3600:
                cursor.execute(
                    "DELETE FROM cv_jobs WHERE last_updated <= datetime('now', '-7 days')"
                )
                conn.commit()
            ...
            cursor.execute(
                f"UPDATE cv_jobs SET status = 'PROCESSING', last_updated = CURRENT_TIMESTAMP WHERE id IN ({placeholders})",
                job_ids,
            )
            conn.commit()
            conn.close()
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                ...
                future_to_job = {
                    executor.submit(process_single_cv, job): job for job in jobs
                }
            ...
            if failures > 0 and successes == 0:
                ...
                if consecutive_fails >= 3:
                    ...
                    print(
                        f"Detected {consecutive_fails} consecutive CV failure batches. Sleeping for 60 seconds to let Gemini API rate limit refresh...\n"
                    )
                    time.sleep(60)
